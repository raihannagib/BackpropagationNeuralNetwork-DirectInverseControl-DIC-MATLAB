...
feature_inverse = normalize(feature_inverse, 'range', [-1, 1]);
target_inverse = normalize(target_inverse, 'range', [-1, 1]);
...
X_train_inverse = table2array(feature_inverse(1:7000, :));
X_train = table2array(feature_inverse(1:7000, :));
y_train_inverse = table2array(target_inverse(1:7000, :));
y_train = table2array(feature_inverse(1:7000, 4));


X_test_inverse = table2array(feature_inverse(7000 + 1:end, :));
X_test = table2array(feature_inverse(7000 + 1:end, :));
y_test_inverse = table2array(target_inverse(7000 + 1:end, :));
y_test = table2array(feature_inverse(7000 + 1:end, 4));


% Inisisalisasi variabel baru 
vektor_u_ANN_train = zeros(length(X_train_inverse), 1);
vektor_u_ref_train = table2array(target_inverse(1:7000, :));
vektor_y_ANN_train = zeros(length(X_train_inverse), 1);
vektor_y_ref_train = table2array(feature_inverse(1:7000, 4));

vektor_u_ANN_test = zeros(length(X_test_inverse), 1);
vektor_u_ref_test = table2array(target_inverse(7000 + 1:end, :));
vektor_y_ANN_test = zeros(length(X_test_inverse), 1);
vektor_y_ref_test = table2array(feature_inverse(7000 + 1:end, 4));
